...
*/
    ...
    fn box_shadow_texture(rect: Rect<f32>,
                          ...
                          rotation: RotationKind,
                          rect_kind: RectangleKind) -> PrimitivePart {
        let (st0, st1) = match rotation {
            RotationKind::Angle0 => {
                (st0, st1)
            }
            RotationKind::Angle90 => {
                (Point2D::new(st0.x, st1.y), Point2D::new(st1.x, st0.y))
            }
            RotationKind::Angle180 => {
                (st1, st0)
            }
            RotationKind::Angle270 => {
                (Point2D::new(st1.x, st0.y), Point2D::new(st0.x, st1.y))
            }
        };
    ...
    fn interp(&self, clipped_rect: &Rect<f32>) -> PrimitivePart {
        ...
        let mut f0 = Point2D::new((clipped_rect.origin.x - self.rect.origin.x) / self.rect.size.width,
                                  (clipped_rect.origin.y - self.rect.origin.y) / self.rect.size.height);

        let mut f1 = Point2D::new((clipped_rect.origin.x + clipped_rect.size.width - self.rect.origin.x) / self.rect.size.width,
                                  (clipped_rect.origin.y + clipped_rect.size.height - self.rect.origin.y) / self.rect.size.height);

        let (f0, f1) = match self.rotation {
            RotationKind::Angle0 | RotationKind::Angle180 => {
                (f0, f1)
            }
            RotationKind::Angle90 | RotationKind::Angle270 => {
                (Point2D::new(f0.y, f0.x), Point2D::new(f1.y, f1.x))
            }
        };
            ...
            ClipKind::ClipOut => {
                ...
                if r == 0.0 {
                    ...
                    if let Some(rect) = self.rect.intersection(&clip.top_left.rect) {
                        ...
                        clipped_part.opacity = Opacity::Translucent;
...
impl FrameBuilder {
    ...
    fn build_prim_parts(&mut self,
        ...
        for prim_index in prim_index_buffer {
            ...
            //let current_part_index = prim_part_list.parts.len();
            ...
            match prim.details {
                ...
                PrimitiveDetails::BoxShadow(ref details) => {
                       ...
                       details.clip_mode == BoxShadowClipMode::None {
                        ...
                        match details.clip_mode {
                            BoxShadowClipMode::None => {
                                ...
                                parts.push(PrimitivePart::solid(details.bs_rect.origin,
                                                                details.bs_rect.size,
                                                                details.color,
                                                                None));
                            }
                            BoxShadowClipMode::Outset => {
                                ...
                                if metrics.br_inner.x > metrics.tl_inner.x &&
                                    ...
                                    // the case!
                                    let clip_in = Clip::from_rect(&details.src_rect, ClipKind::ClipIn);
                            ...
                            BoxShadowClipMode::Inset => {
                                ...
                                // A:
                                parts.push(PrimitivePart::solid(prim.rect.origin,
                                                                Size2D::new(prim.rect.size.width,
                                                                            metrics.tl_outer.y - prim.rect.origin.y),
                                                                details.color,
                                                                None));

                                // B:
                                parts.push(PrimitivePart::solid(metrics.tr_outer,
                                                                Size2D::new(prim.rect.max_x() - metrics.tr_outer.x,
                                                                            metrics.br_outer.y - metrics.tr_outer.y),
                                                                details.color,
                                                                None));

                                // C:
                                parts.push(PrimitivePart::solid(Point2D::new(prim.rect.origin.x, metrics.bl_outer.y),
                                                                Size2D::new(prim.rect.size.width,
                                                                            prim.rect.max_y() - metrics.br_outer.y),
                                                                details.color,
                                                                None));

                                // D:
                                parts.push(PrimitivePart::solid(Point2D::new(prim.rect.origin.x, metrics.tl_outer.y),
                                                                Size2D::new(metrics.tl_outer.x - prim.rect.origin.x,
                    ...
                    match details.clip_mode {
                        ...
                        BoxShadowClipMode::Inset => {
                            let clip = Clip::from_rect(&details.src_rect, ClipKind::ClipOut);
                            prim_part_list.push_parts(&parts, Some(&clip));
                        }
                        BoxShadowClipMode::Outset => {
                            let clip = Clip::from_rect(&details.src_rect, ClipKind::ClipIn);
                            prim_part_list.push_parts(&parts, Some(&clip));
                        }
                    };
...
/*
            for part in &prim_part_list.parts[current_part_index..] {
                if !prim.rect.contains_rect(&part.rect) {
                    println!("ERROR: {:?} {:?}", prim, part);
                }
            }*/
        }
